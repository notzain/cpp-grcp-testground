...
class Oid : public traits::Printable<Oid>
{
  ...
  public:
    ...
    // clang-format off
#define DefineOidTree(oidName, ...) static const auto& oidName##Tree() { static std::array oids = {__VA_ARGS__}; return oids; };
#define DefineOid(oidName, oidString) static const Oid& oidName() { static Oid oid = *parse(oidString); return oid; }
  ...
  DefineOidTree(system, sysDescr(), sysUpTime(), sysContact(), sysName(), sysLocation());
